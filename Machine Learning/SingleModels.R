...
#Single DTree model
single.dtreemodel <- function(dataset){
  ...
  mainDtreemodel <- C5.0(train.data[ , 1:num.cols-1], train.data[ , num.cols])
  prediction_mainDtree <- predict(mainDtreemodel, test.data[ , 1:num.cols - 1], type="class" )
  predictions <- as.numeric(as.character(prediction_mainDtree))
  actuals <- test.data[ , num.cols]
...
single.logisticmodel <- function(dataset){
  dataset <- dataset[sample(nrow(dataset)),]  
  train.data <- dataset[1:round(0.6 * nrow(dataset)), ]  
  test.data  <- dataset[round(0.6 * nrow(dataset))+1:nrow(dataset), ]  
  num.cols <- ncol(train.data)
  ...
  mainlogisticmodel <- glm(formula = formula.init, data = train.data,
                           family = binomial(link="logit"))
  prediction_mainlogistic <- predict(mainlogisticmodel, test.data[ , 1:num.cols - 1], type="response")
  prediction_mainlogistic <- round(prediction_mainlogistic)
  predictions <- as.numeric(as.character(prediction_mainlogistic))
  actuals <- test.data[ , num.cols]
  actuals <- as.numeric(as.character(actuals))
  CrossTable(actuals, predictions, 
             prop.chisq = FALSE, prop.t = FALSE,
